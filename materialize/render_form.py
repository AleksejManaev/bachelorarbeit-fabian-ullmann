...
from .materialize import (
    get_materialize_setting, get_form_renderer, get_field_renderer,
    get_formset_renderer
)
from .text import text_concat, text_value
from .exceptions import MaterializeError
from .utils import add_css_class, render_tag, split_css_classes
from .components import render_icon

FORM_GROUP_CLASS = 'input-field'
ITEM_GROUP_CLASS = 'input-group'


def render_formset(formset, **kwargs):
    """
    Render a formset to a Materialize layout
    """
...
def render_formset_errors(formset, **kwargs):
    """
    Render formset errors to a Materialize layout
    """
    renderer_cls = get_formset_renderer(**kwargs)
...
def render_form(form, **kwargs):
    """
    Render a form to a Materialize layout
    """
...
def render_form_errors(form, type='all', **kwargs):
    """
    Render form errors to a Materialize layout
    """
    renderer_cls = get_form_renderer(**kwargs)
...
def render_field(field, **kwargs):
    """
    Render a field to a Materialize layout
    """
    renderer_cls = get_field_renderer(**kwargs)
    return renderer_cls(field, **kwargs).render()


def render_formfields(form, **kwargs):
    render_cls = get_form_renderer(**kwargs)
    return render_cls(form, **kwargs).render_fields()


def render_formsetfields(formset, **kwargs):
    render_cls = get_formset_renderer(**kwargs)
    return render_cls(formset, **kwargs).render_fields()
...
def render_field_and_label(
    ...
    if layout == 'horizontal':
        if not label_class:
            label_class = get_materialize_setting('horizontal_label_class')
        if not field_class:
            field_class = get_materialize_setting('horizontal_field_class')
        if not label:
            label = '&#160;'
...
def render_form_group(content, css_class=FORM_GROUP_CLASS):
    """
    Render a Materialize form group
    """
...
def is_widget_required_attribute(widget):
    ...
    """
    if not get_materialize_setting('set_required'):
        return False
